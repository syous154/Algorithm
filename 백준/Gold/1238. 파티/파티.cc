...
vector<int> dijkstra(int start, int v, vector<pair<int, int>> graph[]) {
    vector<int> dist(10001); //거리 배열 생성
    for (int i = 1; i <= v; i++) {
        dist[i] = INF;
    }
...
int main()
{
    ...
    vector<pair<int, int>> graph[10001];  // 그래프 형태 선언
    ...
    vector<int> dist2;
    for (int i = 1; i <= n; i++) {
        vector<int> v = dijkstra(i, n, graph);
        dist2.push_back(v[x]);
    }
    int max = 0;
    for (int i = 1; i <= n; i++) {
        if (dist[i] + dist2[i-1] < INF && max < dist[i] + dist2[i-1]) {
            max = dist[i] + dist2[i-1];
        }
    }
    cout << max << '\n';
    return 0;
